package net.aiq9.kaupencoursev2.event;
    ...
    public ActionResult interact(PlayerEntity player, World world, Hand hand, Entity entity, @Nullable EntityHitResult hitResult) {
        ...
        if (entity instanceof SheepEntity && !world.isClient()) {
            ...
            if (player.getMainHandStack().getItem() == ModItems.STRAWBERRY) {
                player.sendMessage(Text.literal(player.getName().getString() + "tried hitting a sheep with a Strawberry?"));
            ...
            if (player.getMainHandStack().getItem() == Items.END_ROD) {
                player.sendMessage(Text.literal(player.getName().getString() + "tried hitting a sheep with an End Rod?"));
